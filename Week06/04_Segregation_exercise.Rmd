...
author: "Aske Svane Qvist and Hania Matera"
...
Good work! The simulation shows that crime is relatively more violent in the town centre and South Western (Eastern?) edge.
...
prob_violent <- list(x = seg$gridx,
                     y = seg$gridy,
                     z = matrix(seg$p[, "Violent crime"], # p stands for probability
                                ncol = ncol))
# You have basically georeferenced the image within data's coordinates
image(prob_violent) # probability
...
p_value <- list(x = seg$gridx,
                ...
                z = matrix(seg$stpvalue[, "Violent crime"]< 0.05,
                                ncol = ncol))
image(p_value) # the significant areas
...
segmap(prob_violent, p_value, 0.05, 0.15) # the thresholds here has nothing to do with p-value (it is a range of violent crime ratios we are interested in) Everything under a probability of 0.05 is low and everthing over 0.15 is high probability areas.
```
