...
>>>>>>> 6fdfe22137327589f0f0ab2b7d51031fb177c6b8

# Take a quick look
head(tree_counts)

# Remove the geometry column from tree_count
...
# Compute the density - trees per square meter
neighborhoods_counts <- mutate(neighborhoods_counts, 
...
# Convert neighborhoods object to a Spatial object (optional) - from data frame to spatial polygon
...
class(neighborhoods_avg_canopy) #its a data farme 
```
...
#there is a negative correlation between the two of -0.3414462
...
tm_shape(neighborhoods2) + 
    tm_polygons("avg_canopy", palette = "Reds", 
        style = "quantile", n = 7, 
...
a <- tm_shape(manhattan) + 
  tm_rgb() 

# I am using tm_rgb instead of tm_raster because I cannot get a visualization of 3 layers stack on top of each other
```
...
#creating a map with two layers overlaid 
map1 <- a+b

map1

#creating a bounding box so that the second map matches manhattan map 
manhattan_box <- st_bbox(manhattan, n = 1)

#canopy map (with a bounding box of manhattan)
map2 <-tm_shape(neighborhoods2,bbox=manhattan_box) + 
...
#tree density map (with a bounding box of manhattan)
...
#arranging the three maps in the same picture - so that they are possible to compare
