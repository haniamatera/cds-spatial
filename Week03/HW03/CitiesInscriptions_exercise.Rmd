...
crs(cities_sf3035) # it is projected (PROJCRS)
```
...
tm_shape(ten_buffers)  +
  ...
  tm_shape(ten_cities) +
  tm_text("Ancient Toponym", size = 0.6, auto.placement = 10) +
  tm_dots(col = "red", 
             size = 0.1) +
  tm_scale_bar(breaks = 0.3,
               text.size = 10,
...
download.file("https://sciencedata.dk/public/b6b6afdb969d378b70929e86e58ad975/EDH_subset_2021-02-15.json", "/Users/hannamatera/Desktop/CDS_new/cds-spatial/Week03/inscriptions.json")

# Load it into R from wherever you put it, and convert into a tibble
list_json <- jsonlite::fromJSON("/Users/hannamatera/Desktop/CDS_new/cds-spatial/Week03/inscriptions.json")
...
i_sm <- inscriptions %>% 
  ...
  mutate(longitude= lapply(str_extract_all(i_sm$longitude,"[0-9.]+"),as.numeric),
         latitude= lapply(str_extract_all(i_sm$latitude,"[0-9.]+"),as.numeric))
...
i <- i %>% mutate(longitude= lapply(str_extract_all(i$longitude,"[0-9.]+"),as.numeric),
         latitude= lapply(str_extract_all(i$latitude,"[0-9.]+"),as.numeric)) 

#dropping NAs
i <- i %>% drop_na(longitude)
i <- i %>% drop_na(latitude)

i <- i %>% filter( longitude > 5 & longitude < 20)
...
The ratio insc_urban/insc_rural is 6.51838. 
